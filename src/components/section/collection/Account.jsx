  ...
  const getNextMint=async()=>{
    if(Contract){
      ...
      for(var j=0;j<TotalTokens;j++){
        const Supply= await Contract.methods.tokenOfOwnerByIndex(account[0],j).call();
            ...
            <div className="collection_items">
              ...
              {/* {console.log(products)} */}
                ...
                {products?.map((product, idx) => (
                  <div key={idx} className="col-lg-4 col-sm-6 col-12" onClick={()=>nav(`/NFT/${product.id}`)} style={{cursor:"pointer"}}>
